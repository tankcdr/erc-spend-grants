...
A widely deployed smart-wallet spend permission (not an ERC) is one token with a recurring period allowance. This proposal allows up to sixteen assets and uses a trailing lookback rather than a period reset.

Native currency uses the [ERC-7528](./eip-7528.md) address rather than `address(0)`. ERC-7528 is Final and is already the native identifier in deployed spend-permission and wallet-permission systems, so wallets and indexers need no second convention. It also keeps `address(0)` available as an unambiguous "unset" value: a zero asset is always invalid, and a zero recipient only ever means "any" in `recipientMode == 1`.
